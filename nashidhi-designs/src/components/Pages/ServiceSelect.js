...
const ServiceSelect = ({ value, onChange, label = "Select Service", sx }) => (
 <FormControl variant="outlined" sx={{ minWidth: 220, background: 'rgba(20,30,50,0.85)', borderRadius: 2, ...sx }}>
  <InputLabel sx={{ color: '#1ec6ff' }}>{label}</InputLabel>
  <Select
   value={value}
   onChange={onChange}
   label={label}
